...
import { Vector3, WaitForSeconds, Quaternion, Time, Object, Coroutine, Mathf, WaitUntil} from "UnityEngine";
import { SyncTransform } from "ZEPETO.Multiplay.Schema";
import { GameObjectStatus, MESSAGE, PositionExtrapolationType, PositionInterpolationType, SyncIndexType, UpdateOwner } from './TypeSyncHelper';
...
export default class TransformSyncHelper extends ZepetoScriptBehaviour {
    ...
    private OnChangeTransform() {
        ...
        const timeOut = timeStampDelta * 0.001;
    ...
    private GetExtraPolationOffset() {
        ...
        const moveDirection: Vector3 = Vector3.Normalize(this.newGet().position - this.prevGet().position);
        const latency = (MultiplayManager.instance.GetServerTime() - this.newGet().timestamp) * 0.001;
    ...
    private* CheckChangeTransform(tick: number) {
        ...
        while (true) {
            ...
            } else if (syncNextFrameCount < syncNextFrameMax) { //Send 10 more frames even if stopped
                this.SendTransform();
                syncNextFrameCount++;
            }
