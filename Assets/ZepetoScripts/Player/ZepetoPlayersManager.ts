...
import { GameObject, Object, Transform, WaitForSeconds} from "UnityEngine";
import { ERROR, PositionExtrapolationType, PositionInterpolationType, SyncIndexType, Tags } from '../SyncHelper/TypeSyncHelper';
...
export default class ZepetoPlayersManager extends ZepetoScriptBehaviour {
    ...
    @SerializeField() private localCharacterNPC: boolean = false;
    ...
    /** multiplayer Spawn **/
    private OnStateChange(state: State, _isFirst: boolean) {
    ...
    private OnJoinPlayer(sessionId: string, player: Player) {
        ...
        if (this.ZepetoPlayerSpawnType == ZepetoPlayerSpawnType.MultiplayerSpawnOnJoinRoom) {
            ...
            if (!this.localCharacterNPC) {
                ...
                else        ZepetoPlayers.instance.CreatePlayerWithUserId(sessionId, player.zepetoUserId, spawnInfo, isLocal);
            }
        }
    }

    private OnLeavePlayer(sessionId: string, _player: Player) {
